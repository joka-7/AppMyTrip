...
class LLMService:
    ...
    async def agent_interaction(
        ...
        system_prompt = (
            ...
            f"{preferences_fragment}"
            "Always include realistic 'map_coordinates' for new locations. For any newly added "
            "activity, also fill 'price' (typical cost in the trip's local currency), 'url' (the "
            "activity's real official website or listing page), and — if it's a historical site — "
            "'hasPodcast': true with a 'podcast_brief' (2-4 sentences of real historical/cultural "
            "context), whenever you can reasonably determine them; otherwise leave them null. "
