...
class Activity(BaseModel):
    ...
    is_kosher: Optional[bool] = Field(None, description="True if the restaurant is Kosher. Mandatory to flag if type is 'food'")
    hasPodcast: bool = Field(False, description="Flag indicating if a historical podcast should be generated for this site")
    podcast_url: Optional[str] = Field(None, description="URL pointing to the generated TTS audio file")
    map_coordinates: Optional[dict[str, float]] = Field(None, description="Dictionary with 'lat' and 'lng' keys for map rendering")
...
class LLMService:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        raise HTTPException(status_code=502, detail=f"LLM API failed after retries: {str(e)}")
    ...
    async def parse_trip_text(cls, raw_text: str) -> TripData:
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail=f"LLM returned invalid schema: {str(e)}")
...
class TTSService:
    """
    Mock service simulating an integration with a Text-To-Speech API (e.g., ElevenLabs).
    In production, this would send text to the API and return an S3/CDN URL.
    """
    @staticmethod
    async def generate_podcast_for_activity(activity_title: str, activity_desc: str) -> str:
        """Simulates TTS generation delay and returns a mock CDN URL."""
        await asyncio.sleep(0.5) # Simulate network delay
        normalized_name = activity_title.lower().replace(" ", "_").replace(":", "")
        return f"https://cdn.tripweaver.ai/podcasts/history_{normalized_name}.mp3"
        ...
        for day in self._trip.days:
            for act in day.activities:
                if act.hasPodcast and not act.podcast_url:
                    act.podcast_url = await TTSService.generate_podcast_for_activity(act.title, act.desc)
        return self
...
async def parse_initial_trip(request: ParseRequest) -> dict:
    ...
    return {
        "trip_data": builder.get_trip().model_dump(),
        "initial_agent_message": agent_msg
    }
...
async def generate_media_endpoint(request: AgentInteractRequest) -> dict:
    ...
    return {
        "trip_data": builder.get_trip().model_dump(),
        "status": "Media generated successfully"
    }
